DELIMITER //
-- 1. 일일 유저 공부시간 정산
CREATE PROCEDURE P_DAILY_USER_STUDY()
BEGIN
	...
	FROM T_IN_LOG
    WHERE STUDY_DATE = CURDATE() - INTERVAL 1 DAY
...
DELIMITER //
-- 2. 일일 방 공부시간 정산
CREATE PROCEDURE P_DAILY_ROOM_STUDY()
BEGIN
...
DELIMITER //
-- 0. 일일 정산프로시저 호출
CREATE PROCEDURE P_DAILY_CALL_PROCEDURE()
BEGIN
	START TRANSACTION;
	-- 1. 유저 일일 공부시간 정산
    CALL P_DAILY_USER_STUDY();
    
    -- 2. 방 일일 공부시간 정산
    CALL P_DAILY_ROOM_STUDY();
